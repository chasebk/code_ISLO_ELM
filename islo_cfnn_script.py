...
from model.main.hybrid_cfnn import IsloCFNN
from utils.IOUtil import read_dataset_file
from utils.SettingPaper import islo_cfnn_paras_final as param_grid
...
test_type = "stability"  ### normal: for normal test, stability: for n_times testggtrace_ram
run_times = None
...
if test_type == "normal":  ### For normal test
    ...
    all_model_file_name = "stability_islo_cfnn"
else:
    pass
...
def train_model(item, dataset, requirement_variables):
    ...
    md = IsloCFNN(root_base_paras=root_base_paras, root_hybrid_paras=root_hybrid_paras, slno_paras=slno_paras)
    md._running__()
    ...
    for _ in range(run_times):
        for loop in range(len(rv_data)):
            ...
            for item in list(ParameterGrid(param_grid)):
                ...
                print("running islo_cfnn with data {} and time {} second".format(loop, time.total_seconds()))