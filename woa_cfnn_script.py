...
from model.main.hybrid_cfnn import WoaCFNN
from utils.IOUtil import read_dataset_file
from utils.SettingPaper import woa_cfnn_paras_final as param_grid
...
if test_type == "normal":  ### For normal test
    ...
    all_model_file_name = "stability_woa_cfnn"
else:
    pass
...
def train_model(item):
    ...
    md = WoaCFNN(root_base_paras=root_base_paras, root_hybrid_paras=root_hybrid_paras, woa_paras=woa_paras)
    md._running__()
...
for _ in range(run_times):
    for loop in range(len(rv_data)):
        ...
        for item in list(ParameterGrid(param_grid)):
            print("running woa_cfnn with data {} and hyper {}".format(loop, item))
            train_model(item)
