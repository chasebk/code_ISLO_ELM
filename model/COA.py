#!/usr/bin/env python
# ------------------------------------------------------------------------------------------------------%
# Created by "Thieu" at 22:28, 28/06/2021                                                               %
#                                                                                                       %
...
class BaseCOA(Root):
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):  # epoch: year
            ...
            pup = p1 * pack_pos[parents[0], :] + p2 * pack_pos[parents[1], :] + n * uniform(self.lb, self.ub, self.problem_size)
            pup_fit = self.get_fitness_position(pup.flatten())
