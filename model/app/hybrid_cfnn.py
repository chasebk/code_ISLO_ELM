...
class HybridCfnn:
    ...
    def predict_using_model(self, data, model):
        ...
        y_pred = add(matmul(hidd, model["w2"]), model["b2"]) + add(matmul(data, model["wc"]), model["bc"])
        return y_pred
    ...
    def solution_to_network(self, solution):
        ...
        b2 = reshape(solution[self.netsize["index_w2"]: self.netsize["index_b2"]], (-1, self.netsize["output"]))
        wc = reshape(solution[self.netsize["index_b2"]: self.netsize["index_wc"]], (self.netsize["input"], self.netsize["output"]))
        bc = reshape(solution[self.netsize["index_wc"]: ], (-1, self.netsize["output"]))
        model = {"w1": w1, "b1": b1, "w2": w2, "b2": b2, "wc": wc, "bc": bc}
        self.weights = {"w1": w1, "b1": b1, "w2": w2, "b2": b2, "wc": wc, "bc": bc}
        return model
    ...
    def objective_function(self, solution):
        ...
        y_pred = add(matmul(hidd, self.weights["w2"]), self.weights["b2"]) + add(matmul(self.X_train, self.weights["wc"]), self.weights["bc"])
    ...
    def experiment(self, optimizer, trials, datadict, series, node, activation, verbose):
        ...
        self.netsize = {
            ...
            "index_b1": (self.lag + 1) * self.node,
            "index_w2": (self.lag + 2) * self.node,
            "index_b2": (self.lag + 2) * self.node + 1,
            "index_wc": (self.lag + 2) * self.node + 1 + self.lag,
            "index_bc": (self.lag + 2) * self.node + 2 + self.lag
        }
        self.problem_size = (self.lag + 2) * self.node + 2 + self.lag
