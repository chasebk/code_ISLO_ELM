...
from model.main.hybrid_cfnn import PsoCFNN
from utils.IOUtil import read_dataset_file
from utils.SettingPaper import pso_cfnn_paras_final as param_grid
...
if test_type == "normal":  ### For normal test
    ...
    all_model_file_name = "stability_pso_cfnn"
else:
    pass
...
def train_model(item, dataset, requirement_variables):
    ...
    md = PsoCFNN(root_base_paras=root_base_paras, root_hybrid_paras=root_hybrid_paras, pso_paras=pso_paras)
    md._running__()
    ...
    for _ in range(run_times):
        for loop in range(len(rv_data)):
            ...
            for item in list(ParameterGrid(param_grid)):
                ...
                print("running pso_cfnn with data {} and time {}".format(loop, time.total_seconds()))
